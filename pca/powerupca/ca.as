...
class cCARound
{
        ...
        case CA_ROUNDSTATE_ROUND:
        {
            ...
            for ( int i = TEAM_PLAYERS; i < GS_MAX_TEAMS; i++ )
            {
                ...
                for ( int j = 0; @team.ent( j ) != null; j++ )
                {
                    ...
                    POWERUP_applyRandomPowerup( @ent );
            ...
            for ( int i = TEAM_ALPHA; i < GS_MAX_TEAMS; i++ )
            {
                ...
                for ( int j = 0; @team.ent( j ) != null; j++ )
                {
                    ...
                    if ( !ent.isGhosting() ) {
                        ...
                        cPowerUp @pwr = @powerUp[ent.client.playerNum];
                        if (pwr == null)
                            continue;
                        pwr.think(ent);
    ...
    void playerKilled( Entity @target, Entity @attacker, Entity @inflictor )
    {
        ...
        {
            clearPowerupState(target);
            cPowerUp @attackerPwr = @powerUp[attacker.playerNum];

			if ( gametype.isInstagib )
			{
				G_PrintMsg( target, "You were fragged by " + attacker.client.name + ( attackerPwr.name.empty() ? "" : "(power: " + attackerPwr.statMessage() ) + ")" + "\n" );
			}
			else
			{
...
bool GT_Command( Client @client, const String &cmdString, const String &argsString, int argc )
{
    ...
    else if ( cmdString == "forcenextround" )
    {
        if (!client.isOperator) {
            G_PrintMsg(client.getEnt(), S_COLOR_RED + "You are not an operator\n");
            return false;
        }
        caRound.newRoundState( CA_ROUNDSTATE_POSTROUND );
...
bool GT_UpdateBotStatus( Entity @ent )
{
    ...
    for ( int i = AI::GetNextGoal( AI::GetRootGoal() ); i != AI::GetRootGoal(); i = AI::GetNextGoal( i ) )
    {
        ...
        // Invisible powerup
        if ( @goal.client != null && powerUp[goal.playerNum] != null && powerUp[goal.playerNum].powerupID == POWERUPID_INVISIBILITY && powerUp[goal.playerNum].ability )
        {
            bot.setGoalWeight( i, 0 );
            continue;
        }
        // Immortal powerup
        if ( @goal.client != null && powerUp[goal.playerNum] != null && powerUp[goal.playerNum].powerupID == POWERUPID_IMMORTALITY && powerUp[goal.playerNum].ability )
        {
            bot.setGoalWeight( i, 0 );
            continue;
        }
...
void GT_ScoreEvent( Client @client, const String &score_event, const String &args )
{
    ...
    {
        POWERUP_applyPowerupByID(client.getEnt(), 0);
    }
...
void GT_ThinkRules()
{
    ...
    for ( int i = 0; i < maxClients; i++ )
    {
        ...
        else
        {
            ...
            if (@powerUp[client.playerNum] == null)
                POWERUP_applyPowerupByID(client.getEnt(), 0);

            client.setHUDStat( STAT_PROGRESS_SELF, powerUp[client.playerNum].getStatFloat() * 100.0f);
...
void GT_InitGametype()
{
    ...
    G_RegisterCommand( "forcenextround" );
